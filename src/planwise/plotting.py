...
def make_growth_plot(df: pd.DataFrame, title: Optional[str] = None) -> alt.Chart:
    ...
    possible, the cumulative net contributions over time.  A single mark type
    (``line``) is used for all series to ensure the returned Altair chart has a
    top‑level ``mark`` property.  When cumulative columns are not present in
    ``df``, they are computed on the fly from the corresponding yearly net
    contribution columns.  Cumulative series are rendered with a dashed stroke
    to distinguish them from the pot values.

    Args:
        df: DataFrame containing projection results from :func:`project_retirement`.
        title: Optional custom title for the chart.  If ``None``, a sensible
            default is used.

    Returns:
        alt.Chart: Altair chart object showing pot growth and, where available,
            accumulated net contributions for each account.
    """
    # Define the mapping between accounts and the column names for pots and
    # yearly net contributions.  The net contribution columns correspond to
    # those produced by :func:`project_retirement` (or similar sample data) and
    # will be used to compute cumulative contributions when the pre‑computed
    # "Accumulated <Account> Net" columns are missing.
    account_info = {
        "LISA": {"pot_col": "Pot LISA", "net_col": "LISA Net"},
        "ISA": {"pot_col": "Pot ISA", "net_col": "ISA Net"},
        "SIPP": {"pot_col": "Pot SIPP", "net_col": "SIPP Employee Net"},
        "Workplace": {"pot_col": "Pot Workplace", "net_col": "Workplace Employee Net"},
    }

    # Determine which accumulated columns already exist; if none exist we will
    # compute them on the fly.  A flag per account is used below to decide
    # whether to compute cumulative sums from the yearly net columns.
    accumulated_cols_available = {
        acc: f"Accumulated {acc} Net" in df.columns for acc in account_info
    }

    # Ensure the data are sorted by age so the cumulative sums make sense
    df_sorted = df.sort_values("Age").reset_index(drop=True)

    # Build a long-form data structure containing both pot values and
    # accumulated net contributions.  Each row will have: Age, Account,
    # Value, and SeriesType ("Pot" or "Accumulated").
    records = []
    for acc, cols in account_info.items():
        pot_col = cols["pot_col"]
        net_col = cols["net_col"]
        # Skip accounts whose pot column is missing
        if pot_col not in df_sorted.columns:
            continue
        # Add pot (nominal) values
        for age, val in zip(df_sorted["Age"], df_sorted[pot_col]):
            records.append({"Age": age, "Account": acc, "Value": val, "Type": "Pot"})
        # Determine the cumulative values; prefer pre‑computed accumulated column
        acc_col_name = f"Accumulated {acc} Net"
        if accumulated_cols_available[acc]:
            cumulative_series = df_sorted[acc_col_name]
        elif net_col in df_sorted.columns:
            # Compute cumulative sum from the yearly net contributions
            cumulative_series = df_sorted[net_col].cumsum()
        else:
            # If neither accumulated nor net column exists, skip this series
            continue
        for age, val in zip(df_sorted["Age"], cumulative_series):
            records.append(
                {"Age": age, "Account": acc, "Value": val, "Type": "Accumulated"}
            )

    # Convert to DataFrame; if no records were created, return an empty chart
    plot_df = pd.DataFrame(records)
    if plot_df.empty:
        # Return an empty chart to avoid Altair errors.  Use a line mark so that
        # the returned chart structure still matches test expectations.
        return alt.Chart(pd.DataFrame({"Age": [], "Value": []})).mark_line()

    # Generate a line chart.  Use strokeDash encoding to distinguish pot values
    # from accumulated contributions.  The order of the dash patterns is
    # explicitly defined so that "Pot" lines are solid and "Accumulated" lines
    # are dashed.  Without specifying the domain and range, Vega‑Lite may pick
