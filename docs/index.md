...
# Simple projection for a 30-year-old
user = pw.core.UserProfile(current_age=30, retirement_age=67, salary=35000)
contrib = pw.core.ContributionRates(isa=0.15)  # 15% to ISA
returns = pw.core.InvestmentReturns(isa=0.05)  # 5% annual return

# Calculate income after tax/NI
income_tax = pw.calculate_income_tax(35000, scotland=False)
ni_due = pw.calculate_ni_due(35000)
take_home = 35000 - income_tax - ni_due
...
df = pw.project_retirement(user, contrib, returns, income)
print(f"Final ISA balance: £{df.iloc[-1]['ISA Balance']:,.0f}")
```
...
# Diversified approach across multiple accounts
user = pw.core.UserProfile(
    current_age=35,
    retirement_age=65,
    salary=55000,
    scotland=True,  # Scottish tax rates
    account_balances=pw.core.AccountBalances(
        isa_balance=15000,
        lisa_balance=8000,
        sipp_balance=25000
...
contrib = pw.core.ContributionRates(
    lisa=4000,  # Max LISA (absolute amount)
    isa=0.10,   # 10% of take-home to ISA
    sipp_employee=0.08,  # 8% employee SIPP
    sipp_employer=0.03,  # 3% employer match
    workplace_employee=0.05,
    workplace_employer=0.03,
    shift_lisa_to_isa=1.0  # Redirect LISA to ISA after 50
)

returns = pw.core.InvestmentReturns(
    lisa=0.06,      # Higher growth for LISA
    isa=0.05,       # Moderate ISA returns
    sipp=0.06,      # Pension growth
    workplace=0.05  # Conservative workplace
)

# Calculate Scottish income tax
income_tax = pw.calculate_income_tax(55000, scotland=True)
ni_due = pw.calculate_ni_due(55000)
take_home = 55000 - income_tax - ni_due
...
# Show final balances
final_row = df.iloc[-1]
print(f"LISA: £{final_row['LISA Balance']:,.0f}")
print(f"ISA: £{final_row['ISA Balance']:,.0f}")
print(f"SIPP: £{final_row['SIPP Balance']:,.0f}")
print(f"Workplace: £{final_row['Workplace Balance']:,.0f}")
print(f"Total: £{final_row['Total Balance']:,.0f}")
```
...
# Run projection (using previous example)
df = pw.project_retirement(user, contrib, returns, income)

# Create visualizations
if pw._PLOTTING_AVAILABLE:
    ...
    pie_chart = pw.make_income_breakdown_pie(income)
    ...
    print("Install plotting dependencies: pip install 'planwise[plotting]'")
```
...
# First run pre-retirement projection
pre_retirement_df = pw.project_retirement(user, contrib, returns, income)

# Then analyze post-retirement phase
post_retirement_df = pw.project_post_retirement(
    pre_retirement_df=pre_retirement_df,
    returns=returns,
    inflation=0.02,
    retirement_age=65,
    state_pension_age=67,
    state_pension_amount=11502  # 2025/26 full state pension
)

print("Post-retirement projection:")
print(post_retirement_df.head())
```
